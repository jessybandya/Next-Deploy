...
export default function RootLayout({ children }) {
  ...
  const [hasSpoken, setHasSpoken] = useState(false); // New state variable
  const microphoneRef = useRef(null);
  const synth = window.speechSynthesis; // Web Speech API synthesizer
  ...
  React.useEffect(() => {
    console.log('Jessy1: ',localStorage.getItem("transcript"))
 }, [transcript])
  ...
  useEffect(() => {
    if (transcript.includes('stop')) {
      ...
      speakText("Page has been refreshed!");
      resetTranscript();
    }
